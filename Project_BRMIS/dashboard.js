...
//  new code 
/* ===================== Layout guard (prevents Archive "collapse") ===================== */
(function injectLayoutGuardStyles () {
  const s = document.createElement("style");
  s.setAttribute("data-layout-guard", "true");
  s.textContent = `
    /* keep these containers at a steady height so layout doesn't jump */
    #announcements .announcement-carousel,
    #archivePanel,
    .news-card .news-list,
    #newsArchivePanel {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const sectionTitles = {
    dashboardSection: "Blood Bank Record Management and Inventory System",
  ...
  function toggleDropdown(btn, dropdown) {
    ...
    if (isOpen) {
      closeDropdown(btn, dropdown);   // close if already open
    } else {
      closeAllDropdowns();           // close others
      openDropdown(btn, dropdown);   // open this one
    }
  }
  ...
  transcriptionBtn?.addEventListener("click", (e) => {
    ...
    e.stopPropagation();
    openSidebarIfCollapsed();
    toggleDropdown(transcriptionBtn, transcriptionDropdown);
  });
...
(function () {
  ...
  // --- Save ONLY (no redirect / no autofill) ---
  registerGo?.addEventListener('click', () => {
    ...
    const saveData = {
      barcode: code,
      destination: regWhere.value,
      type: regSub?.value || null
    };
    localStorage.setItem('registeredBarcode', JSON.stringify(saveData));
...
document.addEventListener('click', async function (e) {
  const t = e.target;
  // safer guard than `instanceof Element` across iframes/shadow roots
...
// changed code
// ===================== Restore saved Announcements/News on page load (with expiry -> archive) =====================
document.addEventListener("DOMContentLoaded", () => {
  ...
  // listKey is "bb_announcements" or "bb_news"
  function moveExpired(listKey){
    const cur = safeParse(listKey);
    const active = [], expired = [];
    const cat = (listKey === "bb_news") ? "News" : "Announcements";

    cur.forEach(a => {
      const isExpired = a?.expiresAt && Number.isFinite(+a.expiresAt) && now > +a.expiresAt;
      if (isExpired && a?.autoArchive) {
        // ensure a.category is present so the archive can filter cleanly
        if (!a.category) a.category = cat;
        expired.push(a);
      } else {
        active.push(a);
      }
    ...
    if (expired.length){
      const old = safeParse("bb_archive");
      // newest first
      save("bb_archive", [...expired, ...old]);
    }
    save(listKey, active);
    return active;
  }

  // 🔴 IMPORTANT: point to the visible/live lists by ID
  const annWrap  = document.getElementById("liveList");       // Active Announcements
  const newsWrap = document.getElementById("liveNewsList");   // Active News

  if (annWrap) {
    const anns = moveExpired("bb_announcements");
    anns.forEach(a => {
  ...
  if (newsWrap) {
    const news = moveExpired("bb_news");
    news.forEach(n => {
      const node = document.createElement("div");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // 1) Top-level items (read label safely; DO NOT mutate .textContent)
  document.querySelectorAll(".nav-list > li > a[data-section]").forEach(a => {
    const li = a.closest("li");
    const label = (
      a.querySelector(".links_name")?.textContent || a.textContent || ""
    ).trim().toLowerCase();
...
// ----- Auto-rotate carousels (Announcements + News) -----
(function () {
  function autoScrollStrip(stripSelector, cardSelector, {
    intervalMs = 3500,
    step = null // pixels to scroll; if null we use card width
  } = {}) {
    ...
    const getStep = () => {
      ...
      const card = strip.querySelector(cardSelector);
    ...
    const tick = () => {
      ...
      if (atEnd) {
        ...
        strip.scrollBy({ left: s, behavior: 'smooth' });
      }
    };
    ...
    // also pause while user is actively scrolling
    let scrollPause;
    strip.addEventListener('scroll', () => {
      paused = true;
      clearTimeout(scrollPause);
      scrollPause = setTimeout(() => { paused = false; }, 600);
    });
  ...
  // Announcements strip
  autoScrollStrip('.announcement-carousel', '.announcement-item', { intervalMs: 3500 });

  // News strip
  autoScrollStrip('.news-list', '.news-item', { intervalMs: 4000 });
})();

// added code
// ----- Announcements Active/Archive tabs -----
document.addEventListener("DOMContentLoaded", () => {
  const activeBtn   = document.getElementById("annActiveTab");
  const archiveBtn  = document.getElementById("annArchiveTab");
  const archiveWrap = document.getElementById("archivePanel");
  const liveWrap    = document.getElementById("liveList");      // ← visible Announcements list
  const archiveList = document.getElementById("archiveList");

  function renderArchive(){
    const data = JSON.parse(localStorage.getItem("bb_archive") || "[]")
      .filter(d => (d?.category || "Announcements") === "Announcements");
    archiveList.innerHTML = "";
    if (!data.length) {
      archiveList.innerHTML = `<div class="announcement-item" style="opacity:.85">
        <div class="announcement-text"><h3>No archived announcements</h3>
        <p>Items with auto-archive will appear here after they expire.</p></div></div>`;
    } else {
      data.forEach(a => {
        const node = document.createElement("div");
        node.className = "announcement-item";
        node.innerHTML = `
          ...
          <div class="announcement-text">
            ...
            <p class="date">${(a.date || "")}</p>
            <p>${(a.content || "")}</p>
            ${a.expiresAt ? `<p style="opacity:.7;font-size:.9em;">Expired: ${new Date(a.expiresAt).toLocaleDateString()}</p>` : ""}
          </div>`;
  ...
  function showActive(){
    if (!liveWrap || !archiveWrap) return;
    archiveWrap.style.display = "none";
    liveWrap.style.display = "";
    if (activeBtn)  activeBtn.style.background  = "#fff";
    if (archiveBtn) archiveBtn.style.background = "#f7f7f7";
  }
  function showArchive(){
    ...
    if (activeBtn)  activeBtn.style.background  = "#f7f7f7";
    if (archiveBtn) archiveBtn.style.background = "#fff";
  }
...
// ----- News & Updates Active/Archive tabs -----
document.addEventListener("DOMContentLoaded", () => {
  const activeBtn   = document.getElementById("newsActiveTab");
  const archiveBtn  = document.getElementById("newsArchiveTab");
  const archiveWrap = document.getElementById("newsArchivePanel");
  const liveWrap    = document.getElementById("liveNewsList");   // ← visible News list
  const archiveList = document.getElementById("newsArchiveList");

  function renderNewsArchive(){
    const data = JSON.parse(localStorage.getItem("bb_archive") || "[]")
      .filter(d => d?.category === "News");
    archiveList.innerHTML = "";
    if (!data.length) {
      archiveList.innerHTML = `<div class="news-item" style="opacity:.85">
        <div class="news-item-title">No archived news</div>
        <div class="news-desc">Expired news will be listed here.</div>
      </div>`;
    } else {
      data.forEach(n => {
        const node = document.createElement("div");
        ...
        node.innerHTML = `
          ...
          <div class="news-date">${(n.date || "")}</div>
          <div class="news-desc">${(n.content || "")}</div>
          ${n.expiresAt ? `<div style="opacity:.7;font-size:.9em;">Expired: ${new Date(n.expiresAt).toLocaleDateString()}</div>` : ""}
        `;
        archiveList.appendChild(node);
      });
    }
...
// new code for settings
/* ================================
   SETTINGS ACCORDION — FINAL (single source of truth)
   - Starts CLOSED
   - Only the clicked card opens; others auto-close (ONE_OPEN_AT_A_TIME = true)
   ================================ */
(() => {
  ...
  if (section.dataset.accInit === '1') return;
  section.dataset.accInit = '1';

  const ONE_OPEN_AT_A_TIME = true;

  const closeCard = (card) => {
    const p = card.querySelector('.accordion-panel');
    card.classList.remove('open');
    if (p) {
      // if previously uncapped, cap it first so we can animate closed
      if (getComputedStyle(p).maxHeight === 'none') p.style.maxHeight = p.scrollHeight + 'px';
      // force reflow so the height above is applied
  ...
  const openCard = (card) => {
    ...
    if (p) {
      ...
      // After the opening transition, uncap height so content can grow
      const onEnd = (e) => {
  ...
  // 1) Start everything CLOSED
  section.querySelectorAll('.setting-card').forEach((c) => {
  ...
  // 2) Single delegated click handler
  section.addEventListener('click', (e) => {
    const header = e.target.closest('.accordion-header');
    if (!header || !section.contains(header)) return;

    const card = header.closest('.setting-card');
    const panel = card?.querySelector('.accordion-panel');
    if (!card || !panel) return;

    if (ONE_OPEN_AT_A_TIME) {
      section.querySelectorAll('.setting-card.open').forEach((other) => {
  ...
  // 3) Keep heights correct while resizing if still capped
  window.addEventListener('resize', () => {
    section.querySelectorAll('.setting-card.open .accordion-panel').forEach((p) => {
...
/* ================================
   PASSWORD eye toggles (Profile)
   ================================ */
(() => {
  const toggles = document.querySelectorAll('#settingsSection .pwd-toggle');
  toggles.forEach((btn) => {
...
/* ================================
   Profile persistence (no backend)
   Uses Employee ID instead of Phone
   ================================ */
(() => {
  const form = document.querySelector('#settingsSection .profile-form');
  if (!form) return;

  const ui = {
    name:        form.querySelector('#profileName'),
    email:       form.querySelector('#profileEmail'),
    role:        form.querySelector('#profileRole'),
    employeeId:  form.querySelector('#profileEmployeeId'),
    avatarInput: form.querySelector('#avatarInput'),
    avatarImg:   form.querySelector('#avatarPreview'),
    saveBtn:     form.querySelector('.save-btn')
  };

  const load = () => {
    const data = JSON.parse(localStorage.getItem('bb_profile') || '{}');
    if (data.name)        ui.name.value = data.name;
    if (data.email)       ui.email.value = data.email;
    if (data.role)        ui.role.value = data.role;
    if (data.employeeId)  ui.employeeId.value = data.employeeId;
    if (data.avatar)      ui.avatarImg.src = data.avatar;
  };

  const save = (extra = {}) => {
    const existing = JSON.parse(localStorage.getItem('bb_profile') || '{}');
    const data = {
    ...
    localStorage.setItem('bb_profile', JSON.stringify(data));
  };

  // Photo preview + persist (as dataURL)
  ui.avatarInput?.addEventListener('change', (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      ui.avatarImg.src = reader.result;
      save({ avatar: reader.result });
    };
    reader.readAsDataURL(file);
  });

  // Save button persists text fields/role
  ui.saveBtn?.addEventListener('click', () => {
    save();
    const txt = ui.saveBtn.textContent;
    ui.saveBtn.textContent = 'Saved';
    setTimeout(() => (ui.saveBtn.textContent = txt), 1200);
  });
...
/* =========================
   GLOBAL password show/hide binder
   - Works for Settings + modals
   ========================= */
(() => {
  function bindPwdToggles(root = document) {
    root.querySelectorAll('.pwd-toggle').forEach(btn => {
      // Don’t double-bind the same button
      if (btn.dataset.bound === '1') return;
      btn.dataset.bound = '1';

      const id = btn.getAttribute('data-target');
      // Prefer finding the target within the same root first
      let input = id ? root.querySelector(`#${CSS?.escape ? CSS.escape(id) : id}`) : null;
      if (!input) input = id ? document.getElementById(id) : null;
      if (!input) return;
  ...
  // Expose for use after dynamic injections (modals)
  window.bindPwdToggles = bindPwdToggles;

  // First pass for anything already on the page (e.g., Settings > Profile)
  document.addEventListener('DOMContentLoaded', () => bindPwdToggles(document));
})();
...
/* =========================
   ...
   ========================= */
(() => {
  const DIALOG   = document.getElementById('amDialog');
  const SHEET    = DIALOG?.querySelector('.am-sheet');
  const TITLE    = DIALOG?.querySelector('#amTitle');
  const BODY     = DIALOG?.querySelector('#amBody');

  const btnAdd   = document.getElementById('amAddUserBtn');
  const btnReset = document.getElementById('amResetPwBtn');
  const btnList  = document.getElementById('amViewUsersBtn');

  const tplAdd   = document.getElementById('tplAddUser');
  const tplReset = document.getElementById('tplResetPw');
  const tplList  = document.getElementById('tplUserList');

  const USERS_KEY = 'bb_users';
  const getUsers  = () => JSON.parse(localStorage.getItem(USERS_KEY) || '[]');
  const setUsers  = (arr) => localStorage.setItem(USERS_KEY, JSON.stringify(arr));

  // Seed sample users once
  if (getUsers().length === 0) {
    setUsers([
      { id:'U-1001', name:'Maria Santos',   email:'maria@demo.com',   role:'head',       employeeId:'EMP-101', lastReset:null },
      { id:'U-1002', name:'Jose Dela Cruz', email:'jose@demo.com',    role:'technician', employeeId:'EMP-102', lastReset:null },
      { id:'U-1ADM', name:'System Admin',   email:'admin@demo.com',   role:'admin',      employeeId:'EMP-001', lastReset:null }
    ]);
  }

  const openDialog = (title, templateEl) => {
    if (!DIALOG || !templateEl) return;
    TITLE.textContent = title;
    BODY.innerHTML = '';                                   // clear old content
    BODY.appendChild(templateEl.content.cloneNode(true));  // inject new content
    DIALOG.setAttribute('aria-hidden', 'false');

    // Bind password eyes INSIDE the freshly injected dialog
    window.bindPwdToggles?.(DIALOG);
  };

  const closeDialog = () => DIALOG?.setAttribute('aria-hidden', 'true');

  // Close via X, backdrop, or data-close-dialog buttons
  DIALOG?.addEventListener('click', (e) => {
    if (e.target.matches('.am-close') || e.target.hasAttribute('data-close-dialog') || e.target === DIALOG) {
      closeDialog();
    }
  });

  // Utility: random temp password
  const genTempPw = (len = 10) => {
  ...
  btnAdd?.addEventListener('click', () => {
    openDialog('Add user', tplAdd);

    // Pre-fill temp password
    const temp = document.getElementById('amAddTempPw');
    if (temp && !temp.value) temp.value = genTempPw();

    const form   = document.getElementById('amAddForm');
    const name   = document.getElementById('amAddName');
    const empId  = document.getElementById('amAddEmpId');
    const email  = document.getElementById('amAddEmail');
    const role   = document.getElementById('amAddRole');
    const errBox = document.getElementById('amAddError');

    form?.addEventListener('submit', (e) => {
      e.preventDefault();
      errBox.textContent = '';
      const users = getUsers();
      const n = name.value.trim(), idEmp = empId.value.trim(), em = email.value.trim().toLowerCase(), r = role.value;
      const exists = users.some(u => u.email === em || u.employeeId === idEmp);
      if (!n || !idEmp || !em || !r) return (errBox.textContent = 'Please fill in all required fields.');
      if (exists)            return (errBox.textContent = 'Email or Employee ID already exists.');

      const newId = 'U-' + Math.floor(1000 + Math.random()*9000);
      users.push({ id:newId, name:n, email:em, role:r, employeeId:idEmp, lastReset:new Date().toISOString() });
      setUsers(users);
      closeDialog();
    });
  });
  ...
  btnReset?.addEventListener('click', () => {
    openDialog('Reset password', tplReset);

    // Populate users
    const sel  = document.getElementById('amResetUser');
    const temp = document.getElementById('amResetTempPw');
    if (temp && !temp.value) temp.value = genTempPw();
    ...
    const form = document.getElementById('amResetForm');
    const msg  = document.getElementById('amResetMsg');
    form?.addEventListener('submit', (e) => {
      e.preventDefault();
      const id = sel.value;
      const t  = temp.value.trim() || genTempPw();
      const arr = getUsers();
      const i = arr.findIndex(u => u.id === id);
      if (i >= 0) { arr[i].lastReset = new Date().toISOString(); arr[i].tempPassword = t; setUsers(arr); }
      msg.textContent = `Temporary password set to: ${t}`;
      setTimeout(() => closeDialog(), 900);
    });
  });

  btnList?.addEventListener('click', () => {
    openDialog('User list', tplList);
    renderUsersTable();
  });

  function renderUsersTable(){
    const wrap = document.getElementById('amUsersTableWrap');
    const users = getUsers();
    if (!users.length) { wrap.innerHTML = '<p class="hint">No users yet.</p>'; return; }
    wrap.innerHTML = `
      <table class="table-sm">
        <thead><tr>
          <th>Name</th><th>Email</th><th>Role</th><th>Employee ID</th><th>Last reset</th>
        </tr></thead>
        <tbody>
          ${users.map(u => `
            <tr>
              ...
              <td>${u.lastReset ? new Date(u.lastReset).toLocaleString() : '—'}</td>
            </tr>`).join('')}
        </tbody>
      </table>`;
  }
